...
def main(opts):
    ...
    if rank == 0:
        logger = Logger(logdir_full, rank=rank, debug=opts.debug, summary=opts.visualize, print_flag=opts.print)
    else:
        logger = Logger(logdir_full, rank=rank, debug=opts.debug, summary=False, print_flag=opts.print)
    ...
    print(f"Dataset: {opts.dataset}, Train set: {len(train_dst)}, "
                f"Val set: {len(val_dst)}, n_classes {opts.num_classes}")
    print(f"Total batch size is {opts.batch_size * world_size}")
    ...
    while cur_epoch < opts.epochs and not opts.test:
        ...
        if opts.output_aux and opts.model == 'bisenetv2':
            print(f"End of Epoch {cur_epoch+1}/{opts.epochs}, Average Loss={epoch_loss[0] + epoch_loss[1]:.4f}, "
        ...
        if (cur_epoch + 1) % opts.val_interval == 0:
            ...
            print(f"End of Validation {cur_epoch}/{opts.epochs}, Validation Loss={val_loss}")
    ...
    print(f"*** End of Test on all, Total Loss={val_loss}")
