...
class Logger:
    ...
    def __init__(self, logdir, rank, type='torch', debug=False, filename=None, summary=True, step=None, print_flag=False):
        self.logger = None
        self.type = type
        self.rank = rank
        self.step = step
        ...
        logging.basicConfig(filename=filename, level=logging.DEBUG, format=f'%(levelname)s:rank{rank}: %(message)s')

        if rank == 0:
            os.makedirs(self.logdir_results, exist_ok=True)
            print_(f"[!] starting logging at directory {logdir}")
            if self.debug_flag:
                print_(f"[!] Entering DEBUG mode")
        print_(f"[!] Entering DEBUG mode")
